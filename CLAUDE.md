...
- **Notificación por correo — eliminada (2026-09-01):** hasta esta fecha, además de WhatsApp, cada pedido nuevo intentaba enviarse por `mail()` a `NOTIFICACION_EMAIL`. La clienta pidió no usar ese canal ("ya no se hará"), así que se quitó por completo: la función `notificar_pedido_por_correo()` y su llamada en `api/pedidos.php`, la constante `NOTIFICACION_EMAIL` en `config/db.php`, y la variable de ejemplo en `.env.example`. El único aviso de pedido nuevo es WhatsApp (sección 6.4) más la lista en `/admin/pedidos.php`.
