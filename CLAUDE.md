...
| Catálogo y taxonomía de categorías | **Base de datos MySQL normalizada** (`database/schema.sql`), consultada en vivo mediante **PHP 8.2 + PDO** desde `/api/*.php` (sitio público) y directamente desde `/admin` | Con 12 categorías y ~80 subcategorías, un modelo relacional evita duplicación e inconsistencias que un JSON plano ya no podía sostener con orden |
...
**Categoría "Arreglos Cumpleaños" eliminada; subcategorías consolidadas en TODAS las categorías EXCEPTO "Ramos Florales" (2026-08-31/09-01):** la clienta pidió quitar la clasificación de subcategoría en varios departamentos. Primer intento (2026-08-31) interpretó mal el alcance y consolidó también "Ramos Florales" — la clienta corrigió por WhatsApp al día siguiente: *"Las sub categoría de ramos está bien. El resto se quita. Y eso de arreglos de cumpleaños, puedes volver a poner las subcategorias de ramos?"*. Estado final:
  - `cumpleanos` (id 4): sin productos cargados, se eliminó por completo (cascada borra sus 6 subcategorías vacías) — se mantiene eliminada, no se restauró.
  - `ramos-florales`: restaurada a sus 8 subcategorías originales (Rosas, Girasoles, Hortensias, Tulipanes, Lirios, Gerberas, Mini Rosas, Mixtos) con los 59 productos de vuelta en su subcategoría original (mapeo exacto tomado del backup del primer intento).
  - Las otras 13 categorías (Arreglos en Base, Caballero, Infantil, Desayuno Sorpresa, Aniversario, Complementos, Graduaciones, Bodas, Fúnebres, Flores Preservadas, Globos, Amor y Romance, Peluches) se consolidaron cada una en una única subcategoría `general` ("General"), reasignando sus productos existentes antes de borrar las subcategorías viejas — ningún producto se eliminó. `flores-amarillas` y `girasoles` ya no tenían subcategorías ni productos, se dejaron igual.
  - Motivo del "General" en vez de dejar `subcategoria_id` vacío: la columna es `NOT NULL` (FK a `subcategorias`), así que todo producto necesita alguna subcategoría — "General" es solo un valor interno para que el panel admin siga permitiendo cargar productos nuevos en esas categorías; no se expone como clasificación real.
  - Backups: primer intento en `database/backups/cumpleanos_y_subcategorias_ramos_florales_20260901_055801.sql`; corrección (consolidación de las 13 categorías + restauración de Ramos Florales) en `database/backups/consolidar_subcategorias_excepto_ramos_20260901_061539.sql`.
  - En el sitio público, `renderChips()` de `js/productos.js` (`categoria.html`) ya no hardcodea slugs de categoría: oculta la fila de chips de subcategoría cuando la categoría tiene 1 o 0 subcategorías (todas menos Ramos Florales), y la muestra normalmente cuando tiene más de una (hoy, solo Ramos Florales).
  - En `/admin/productos.php`, el desplegable "Categoría / Subcategoría" muestra una sola opción "General" para las 13 categorías consolidadas, "Rosas/Girasoles/.../Mixtos" para Ramos Florales como antes, y ya no existe la categoría Cumpleaños en ningún desplegable del panel.

**Ocultar la palabra "General" (2026-09-01):** la subcategoría interna "General" (nota anterior) se filtró a dos lugares visibles que la clienta no quería ("no me gusta que se lea general así"): el `<select>` de categoría/subcategoría de `admin/productos.php` y, más grave, la ficha pública de producto (`producto.html`, `.card-occasion` en `js/productos.js`), que mostraba literalmente "General" bajo el nombre del arreglo para cualquier producto fuera de Ramos Florales. Se corrigió sin tocar la base de datos (la subcategoría "General" sigue existiendo, solo se disimula en la UI):
  - `admin/productos.php`: cuando una categoría tiene una sola subcategoría, el `<select>` la muestra como una opción plana con el **nombre de la categoría** (ej. "Arreglos en Base") en vez de un `<optgroup>` con la opción "General" adentro — se sigue guardando el mismo `subcategoria_id` por debajo. La columna "Subcategoría" de la tabla del catálogo muestra "—" en vez de "General". El `<optgroup>` con subcategorías reales solo se sigue usando para Ramos Florales (la única categoría con más de una subcategoría hoy).
  - `js/productos.js`: `.card-occasion` en la ficha de producto usa el nombre de la categoría como respaldo cuando la subcategoría es "General", en vez de mostrar "General" tal cual.

**Subcategorías de "Arreglos en Base" eliminadas (2026-08-27):** la clienta pidió quitar las 6 subcategorías de bases (Vidrio, Cerámica, Madera, Acrílico, Cartón Premium, Canastas) — tenían 27 productos ya cargados, que se eliminaron junto con ellas (decisión explícita: "Eliminar subcategorías y sus productos", no solo ocultarlos). Backup completo de las 6 subcategorías y los 27 productos en `database/backups/arreglos_en_base_subcategorias_y_productos_20260827_042728.sql` antes del borrado. La categoría padre "Arreglos en Base" quedó sin subcategorías ni productos — se marcó `visible=0` (no se borró) para que no aparezca vacía en el menú/home; sigue en `/admin/categorias.php` por si se reutiliza con subcategorías nuevas más adelante.

**Categoría visible sí/no (columna `categorias.visible`, 2026-08-26):** antes, ocultar una categoría del menú "Catálogo" y de la grilla de categorías del home dependía de un truco frágil (`orden >= 90` = "huérfana") sin ningún control en el panel — así fue como "Flores Amarillas" quedó invisible por accidente al creársele con `orden=92`. Ahora `admin/categorias.php` tiene un checkbox "Visible" por categoría (columna `categorias.visible`, `TINYINT(1)` independiente de `orden`, que sigue siendo solo el orden de aparición): desmarcarlo oculta la categoría del menú del header (`js/main.js`) y de la grilla de categorías del home (`initCategoryGrid()` en `js/productos.js`), sin borrarla ni afectar sus productos — su página (`categoria.html?slug=...`) sigue siendo accesible por enlace directo. La migración inicial marcó `visible=0` exactamente en las categorías que ya estaban ocultas por el truco de `orden` (Amor y Romance, Peluches, Girasoles), preservando el estado visual previo.

**Producto (tabla `productos`):** `nombre`, `slug`, `descripcion_corta` (flores principales, se muestra en la tarjeta), `descripcion` (texto largo, ficha de producto), `imagen`, `incluye` (texto libre, ej. "Tarjeta personalizada y empaque premium"), `precio` (único, ver nota abajo), `entrega_disponible` / `retiro_tienda_disponible` (booleanos independientes), `disponible`, `destacado`.

**Chips de categoría en `catalogo.html` (2026-08-13):** el `<select id="filter-categoria">` se reemplazó por una fila de píldoras con scroll horizontal (`.category-chips`/`.category-chip` en `styles.css`, lógica en `initCatalog()` de `js/productos.js`) — un toque para filtrar en vez de abrir un `<select>` nativo, y en móvil se navegan arrastrando. El estado de la categoría activa vive en la variable `categoriaActual` (ya no en `selCategoria.value`, ese elemento no existe más); `poblarSubcategorias()` y `render()` la leen directo. Cada chip usa el emoji de `categorias.icono` si existe (hoy la semilla no tiene íconos cargados, así que los chips se ven solo con texto — probar poniéndole un emoji a una categoría desde `/admin/categorias.php` para ver el ícono en el chip). Búsqueda, subcategoría y orden siguen en `.filter-bar` sin cambios.

**Chips de subcategoría en `categoria.html` (2026-08-26):** la clienta pidió poder filtrar por tipo de flor (Rosas, Lirios, Gerberas...) al entrar a una categoría desde el home (ej. "Ramos Florales"), en vez de que todo saliera mezclado bajo el orden por defecto "Recomendados" — "no tiene sentido que yo esté clasificando a la hora de subir y aquí se mezcló todo". Se agregó una fila de chips (`#subcategory-chips`, mismas clases `.category-chips`/`.category-chip` que ya usaba `catalogo.html`) dentro de `initCategoryPage()` en `js/productos.js`, que filtra vía el parámetro `subcategoria` que `api/productos.php` ya soportaba.

**Precio único por producto (2026-08-26):** el catálogo tuvo hasta 4 variantes de talla (S/M/L/XL) con precio propio por talla (tabla `producto_variantes`). La clienta pidió quitar ese sistema ("quitar segmentos de bases") y dejar un precio único por producto — columna `productos.precio`. Los productos que ya tenían varias tallas se migraron al precio de la talla M (o la talla disponible más cercana a M si no tenía M); backup de los datos previos en `database/backups/producto_variantes_20260826_073258.sql`. `admin/productos.php` ahora pide un solo campo "Precio", y `api/producto.php`/`api/productos.php` devuelven `precio` directo (ya no `variantes`/`precioDesde`).

- Filtro del catálogo por categoría → subcategoría (chips/selects) y orden por precio.
- Botones de la ficha de producto: **"Agregar al carrito"** y **"Comprar ahora"** (agrega el producto y navega directo a `/carrito.html` para revisar y enviar por WhatsApp).

**Tarjeta de producto — solo precio (2026-08-26):** en catálogo/categoría/destacados/relacionados, la tarjeta pública (`productCardHtml()` en `js/productos.js`) muestra únicamente el precio del arreglo, sin nombre ni código de ramo visibles — el mismo día se probó primero con el código de ramo en vez del nombre, pero la clienta pidió después quitarlo también y dejar solo el precio. El nombre real del producto se conserva en el `alt` de la imagen y el `aria-label` del enlace (accesibilidad), y el código de ramo sigue existiendo igual que antes para el carrito, el mensaje de WhatsApp y `/admin/codigos.php` (`codigoRamo()` en `js/cart.js`, ya no exportada al no necesitarla `productos.js`) — el cambio es solo de qué se ve en la tarjeta, no del sistema de códigos. Los chips de talla (`.card-tallas`/`.talla-chip`) se quitaron junto con el sistema de tallas de la nota anterior. Mismo componente reutilizado en destacados del home, catálogo, categoría y relacionados de `producto.html`.

**Alta de productos simplificada a Categoría + Foto + Precio (2026-08-27):** como la tarjeta pública ya no muestra nombre ni descripción (nota anterior), la clienta pidió simplificar también el formulario de "Agregar producto nuevo" del panel a lo esencial. El nombre se sigue generando por dentro (automático, nunca se ve en el sitio) y la carga rápida de 5 productos a la vez quedó deshabilitada — detalle completo en `admin/CLAUDE.md`.
...
- **Hora de entrega solo como referencia (2026-09-03):** Aldo reportó por WhatsApp que los clientes agendaban libremente cualquier hora y todos coincidían en la misma franja (ej. 10am), saturando la operación del día. Se decidió (ver pregunta al cliente) no limitar horarios por ahora — el campo Hora sigue siendo texto libre y opcional — y en su lugar se agregó un aviso (`.field-hint` bajo el input) aclarando que la hora es aproximada y se confirma por WhatsApp según disponibilidad. No resuelve el amontonamiento de fondo, solo baja la expectativa del cliente; si el problema persiste, la alternativa evaluada y descartada por ahora fue franjas horarias con cupo limitado por día.
- **Costo de delivery por zona con mapa (2026-09-03, editor por puntos desde 2026-09-04):** Aldo también reportó no tener forma de cobrar el delivery. Se agregó un sistema de zonas de delivery con precio propio, definidas por Claudia como polígonos sobre un mapa de El Progreso (`/admin/zonas-delivery.php`, tabla `zonas_delivery` — ver `admin/CLAUDE.md`). Cada zona se arma agregando sus puntos/vértices uno por uno (tocando el mini-mapa, con la ubicación GPS del celular parado en cada esquina, o escribiendo coordenadas a mano) — la primera versión usaba la herramienta de "dibujar arrastrando" de Leaflet.draw, pero se reemplazó porque a la clienta no le funcionaba bien (ver detalle en `admin/CLAUDE.md`). En `/carrito.html`, al elegir "Delivery a domicilio" aparece un mapa (`#delivery-map-block`, `js/delivery-map.js`, Leaflet + OpenStreetMap vía CDN, sin API key) donde el cliente arrastra un pin hasta su ubicación; el sitio detecta con point-in-polygon en qué zona cae (o ninguna) y suma su precio al total (`renderTotales()` en `js/cart.js`, fila "Delivery (Zona)" en el resumen). Decisiones de negocio confirmadas con el cliente:
  - Si el mapa **no tiene ninguna zona configurada todavía** (aún no se ha dibujado ninguna desde el panel), el mapa ni se muestra — el delivery se coordina como antes, sin costo calculado, para no romper el checkout mientras Claudia configura las zonas.
  - Si el pin cae **fuera de todas las zonas dibujadas**, no se permite continuar: se le pide al cliente escribir directo por WhatsApp para coordinar. `validarCamposObligatorios()` en `js/cart.js` bloquea el envío en ese caso (toast + scroll al mapa), pero solo cuando ya hay al menos una zona configurada (`LiriosDelivery.zonasConfiguradas`).
  - El precio del delivery **nunca se confía en lo que manda el navegador**: el cliente solo envía el id de zona detectado (calculado en `js/delivery-map.js`) más las coordenadas del pin; `api/pedidos.php` vuelve a hacer el point-in-polygon del lado servidor contra el polígono real en la base de datos y toma el precio de ahí (`puntoEnPoligono()`, mismo algoritmo ray-casting reimplementado en PHP).
  - `pedidos` ganó las columnas `zona_delivery_id` (FK a `zonas_delivery`, `ON DELETE SET NULL`), `zona_delivery_nombre` (copia del nombre al momento del pedido, sobrevive si se borra la zona) y `costo_delivery`. El mensaje de WhatsApp (`buildOrderMessage()` en `js/whatsapp.js`) agrega una línea `*Zona de entrega:* ... / *Costo delivery:* ...` solo cuando aplica.
  - Migración aplicada en caliente contra la base remota de producción (no solo en `database/schema.sql`) con backup previo de `pedidos` en `database/backups/pedidos_pre_zonas_delivery_20260904_064223.sql`.
- **Mapa de zonas quitado del carrito por ahora; aviso de costo por SweetAlert (2026-09-04):** el mismo día que se lanzó el punto anterior, el cliente pidió que el comprador **ya no elija su ubicación en un mapa** dentro de `/carrito.html`. Se quitó `#delivery-map-block` de `carrito.html` y su lógica de `js/cart.js` (`deliveryCost()`, la fila "Delivery (Zona)" del resumen, el bloqueo por "fuera de cobertura" en `validarCamposObligatorios()`) y el `<script>`/`<link>` de Leaflet de esa página — **`js/delivery-map.js` no se borró**, solo dejó de importarse desde `cart.js`, por si se retoma más adelante. El delivery vuelve a coordinarse 100% por WhatsApp sin costo calculado en el carrito (como antes del 2026-09-03); `/admin/zonas-delivery.php`, `api/zonas-delivery.php` y las columnas `zona_delivery_*`/`costo_delivery` de `pedidos` no se tocaron (siguen ahí, simplemente no se usan desde el sitio público por el momento). En su lugar, al elegir "Delivery a domicilio" en `#customer-delivery-type` aparece un SweetAlert2 (`Swal.fire(...)` en `initCartPage()` de `js/cart.js`, cargado vía CDN en `carrito.html`) avisando que el costo del envío se coordina y confirma por WhatsApp y que el total podría cambiar — reemplaza el aviso automático de precio por una advertencia explícita, ya que ahora no hay ningún monto de delivery sumado al total mostrado en el carrito.
- **Aviso de costo de delivery pendiente en el mensaje de WhatsApp (2026-09-06):** como ya no se calcula ningún costo de delivery en el carrito (nota anterior), el `*Total:*` del mensaje solo reflejaba el subtotal de productos sin que quedara explícito para quien lo recibe. `buildOrderMessage()` en `js/whatsapp.js` agrega una línea `*Costo de delivery:* Pendiente de confirmar por WhatsApp (no incluido en el total)` justo después de `*Delivery o retiro:*`, solo cuando `tipoEntrega` es "Delivery a domicilio". Se aprovechó para quitar los parámetros `zonaDeliveryNombre`/`costoDelivery` de `datosCliente` (ya no los envía nadie desde que se quitó el mapa).
- **Hora de entrega restringida al horario de la tienda (2026-09-04):** además de lo anterior, el cliente pidió que solo se puedan elegir horas dentro del horario de atención (sección 2: lunes a sábado 8:30 a.m.–6 p.m., domingo 10 a.m.–4 p.m.), en vez del texto libre descrito en la nota "Hora de entrega solo como referencia" de arriba (2026-09-03) — esa nota queda parcialmente obsoleta: el campo sigue opcional y con el mismo `.field-hint` de "hora aproximada, se confirma por WhatsApp", pero ya no es texto libre. `#customer-delivery-time` pasó de `<input type="time">` a `<select>`, poblado por `poblarHorasEntrega()` en `js/cart.js` con franjas de 30 minutos dentro del horario del día elegido en `#customer-delivery-date` (recalculado en cada cambio de fecha, según el día de la semana de esa fecha — domingo usa la franja corta, el resto la franja larga). Si todavía no hay fecha elegida, el select solo muestra "Elige primero la fecha" y queda deshabilitado.
...
- Ejemplo de mensaje generado (`buildOrderMessage()` en `js/whatsapp.js`, formato pedido por la clienta el 2026-08-22):
  ...
  *Referencia del arreglo:*
  https://liriosfloristeria.com/foto.php?p=ramo-rosas-rojas
  ```
...
- **Referencia del arreglo = foto real, no placeholder (2026-08-22):** `buildReferenciaLineas()` en `js/whatsapp.js` arma un enlace a la foto real de cada item del carrito (ver nota 2026-09-01 abajo sobre el formato exacto del enlace). Con 1 producto en el carrito sale un solo enlace en "Referencia del arreglo"; con 2+ sale una lista "Referencia de los arreglos:" con un enlace por producto, precedido de su nombre. Si algún item no tiene foto (`imagen` es `null`), simplemente no aparece en la lista; si ningún item del carrito tiene foto, la línea dice "(sin foto disponible)".
- **Enlace corto vía `/foto.php` en vez de la ruta directa a la imagen (2026-09-01):** hasta esta fecha el enlace de "Referencia del arreglo" era la ruta completa a la imagen (ej. `.../images/productos/05786acc-b908-49c3-979e-0ddd1fa723f9.webp`, construida con `new URL(item.imagen, location.href)`). Una clienta (Gabriela, pedido #30) reportó no poder ver la foto: el link le llegó cortado justo antes del `.webp` final — un link largo, con muchos guiones y caracteres hexadecimales parecidos entre sí, terminado en una extensión de archivo que se puede perder al copiar/reenviar el mensaje. Se verificó que no era un bug del sitio (la ruta en la base de datos y el archivo en el servidor estaban correctos, la URL completa cargaba bien), pero el formato del enlace era frágil igual. Se agregó `/foto.php` (raíz del sitio, solo lectura, sin autenticación — mismo criterio que `/api/*.php` de solo lectura): recibe `?p=<slug-del-producto>`, busca `productos.imagen` por `slug` y responde con un redirect 302 a la ruta real; si el slug no existe o el producto no tiene foto, responde 404 en vez de fallar en silencio. `fotoUrl()` en `js/whatsapp.js` (antes `absoluteUrl()`) arma el enlace como `foto.php?p=<slug>` usando `item.id` del carrito (que ya guarda el slug del producto, ver `js/productos.js`) — el enlace final ya no termina en una extensión de archivo que se pueda recortar, y es bastante más corto. Se resuelve igual que antes contra `location.href` (no `location.origin`), así que sigue funcionando igual en la raíz del dominio o en una subcarpeta.
...
- **Notificación por correo — eliminada (2026-09-01), reintroducida (2026-09-06):** hasta 2026-09-01, además de WhatsApp, cada pedido nuevo intentaba enviarse por `mail()` a `NOTIFICACION_EMAIL`. La clienta pidió no usar ese canal ("ya no se hará"), así que se quitó por completo: la función `notificar_pedido_por_correo()` y su llamada en `api/pedidos.php`, la constante `NOTIFICACION_EMAIL` en `config/db.php`, y la variable de ejemplo en `.env.example`. El 2026-09-06 se pidió reactivarlo, esta vez para avisar a `Aldonoeperezmoreno2@gmail.com` (correo de Aldo, no de Claudia) cada vez que se registra un pedido. Reimplementado desde cero, no como un revert del código viejo:
  - `mail()` nativo no sirve para autenticar contra una cuenta de Gmail real (Gmail exige SMTP con usuario/contraseña), así que se vendorizó **PHPMailer** sin Composer — solo los 3 archivos fuente (`PHPMailer.php`, `SMTP.php`, `Exception.php`) descargados directo del release `v6.9.1` a `/lib/PHPMailer/`, incluidos con `require_once` (sin `vendor/autoload.php`, consistente con "sin build step" de la sección 4). `/lib` tiene su propio `.htaccess` (`Require all denied`, igual que `/config`) y está bloqueado en `robots.txt`.
  - `config/mailer.php` define `notificar_pedido_por_correo(int $pedidoId, array $pedido)`, llamada desde `api/pedidos.php` justo después del `commit()` de la transacción — nunca antes, para no notificar un pedido que no llegó a guardarse. Igual que el código anterior, es **fire-and-forget y nunca bloquea la venta**: cualquier error de SMTP se atrapa (`catch (Throwable $e)`) y solo se registra con `error_log()`, la respuesta al navegador sigue siendo `201` normalmente.
  - Credenciales nuevas en `.env`/`.env.example`: `SMTP_HOST` (default `smtp.gmail.com`), `SMTP_PORT` (default `587`), `SMTP_USER`, `SMTP_PASS`, `NOTIFICACION_EMAIL`. `SMTP_USER`/`SMTP_PASS` son la cuenta de Gmail remitente y su **contraseña de aplicación** de 16 caracteres (`myaccount.google.com/apppasswords`, requiere verificación en 2 pasos) — nunca la contraseña normal de la cuenta. Si falta cualquiera de las tres (`SMTP_USER`, `SMTP_PASS`, `NOTIFICACION_EMAIL`), la función no hace nada (mismo criterio que el chequeo de `function_exists('imagewebp')` para WebP: si el requisito no está configurado, se omite en silencio en vez de fallar la petición).
  - El cuerpo del correo (`construir_cuerpo_correo_pedido()` en `config/mailer.php`) es texto plano con el mismo contenido que el mensaje de WhatsApp de la sección 6.4 (items con código de ramo, total, datos del cliente, zona de delivery si aplica) más un enlace a `/admin/pedidos.php`; los campos vacíos también salen como `__`, igual que en WhatsApp.
  - El único aviso de pedido nuevo sigue siendo WhatsApp + `/admin/pedidos.php` como canales principales (sección 6.4); el correo es un aviso adicional a Aldo, no reemplaza nada.
...
- **SEO:** `sitemap.php` (dinámico: páginas fijas + un `<url>` por producto disponible, URL base autodetectada del request) y `robots.txt` (bloquea `/admin`, `/api`, `/config`, `/database`, `/lib`); JSON-LD `Florist` en `index.html` con dirección/horario/redes. El dominio real ya está en producción (`liriosfloristeria.com`, confirmado 2026-09-01) y ya está puesto tanto en el JSON-LD de `index.html` como en el `Sitemap:` de `robots.txt` — no quedó ningún placeholder pendiente de reemplazar.
...
/foto.php                 → Redirect corto a la foto de un producto (?p=slug), usado en el mensaje de WhatsApp (ver sección 6.4)
...
/js
  ...
  admin.js                       (modales, confirmaciones y alertas del panel — ver admin/CLAUDE.md, solo lo carga /admin)
/config
  db.php                    (conexión PDO compartida a MySQL — la usan /api/*.php y /admin)
  mailer.php                 (aviso por correo de "pedido nuevo" vía SMTP de Gmail, ver sección 6.6bis)
/lib
  PHPMailer/                (PHPMailer v6.9.1 vendorizado sin Composer, solo lo incluye config/mailer.php)
/api                        → Endpoints PHP para el sitio público (JSON, MySQL en vivo)
  ...
  producto.php                 (detalle de un producto por ?slug=, con relacionados — solo lectura)
...
/admin                     → Panel de administración (ver 7.1), PROTEGER con contraseña
  ...
  productos.php                       (CRUD de productos y su precio)
...
- [x] Modelo de datos para precio y campos por producto (nombre, precio, descripción corta, "incluye", disponibilidad) → resuelto (2026-07-25): base de datos MySQL normalizada, ver sección 4 (el sistema de tallas S/M/L/XL que tuvo este modelo se quitó después, ver nota 2026-08-26 en sección 6.1)
...
- [x] Catálogo real de productos → resuelto: 182 productos cargados con foto y precio real (ya no son los 7 de ejemplo/placeholder iniciales)
- [ ] Textos de la sección "Nosotros" (historia de la floristería) y fotos para esa página
- [x] HTTPS/SSL → resuelto: el sitio corre en producción en `https://liriosfloristeria.com` con SSL activo
- [x] Hosting con MySQL/MariaDB → resuelto: el sitio ya opera contra una base de datos MySQL remota real (credenciales en `.env`, fuera del repo)
- [x] Dominio final → resuelto: `liriosfloristeria.com`, ya puesto en el JSON-LD de `index.html` y en el `Sitemap:` de `robots.txt` (ver nota 2026-09-01 en la sección 6.6bis)
- [x] GD de PHP con soporte WebP en el hosting final → resuelto: confirmado funcionando en producción, las fotos subidas desde `/admin` ya se convierten a WebP automáticamente
...
4. **Fase 3:** feed real de Instagram, sistema de reseñas de clientes. (La notificación de pedidos por correo, que estaba planteada aquí, se descartó — ver nota 2026-09-01 en la sección 6.6bis: WhatsApp es el único canal de aviso.)