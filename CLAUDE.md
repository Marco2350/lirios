...
**Tarjeta de producto — código de ramo en vez de nombre (2026-08-26):** en catálogo/categoría/destacados/relacionados, la clienta pidió mostrar el código de ramo (ej. "RM-4K2P9") y el precio en la tarjeta, sin el nombre del producto visible. `productCardHtml()` en `js/productos.js` calcula el código con `codigoRamo()` (exportada desde `js/cart.js`, misma función determinista que ya generaba el código al agregar al carrito) a partir de `p-${slug}` — el mismo producto siempre muestra el mismo código, coincidente con el que después aparece en el carrito, el mensaje de WhatsApp y `/admin/codigos.php`. El nombre real del producto se conserva en el `alt` de la imagen y el `aria-label` del enlace, solo se ocultó de la visual. Los chips de talla (`.card-tallas`/`.talla-chip`) se quitaron junto con el sistema de tallas de arriba. Mismo componente reutilizado en destacados del home, catálogo, categoría y relacionados de `producto.html`.
